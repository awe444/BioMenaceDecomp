...
/* On-disk huffman node: two uint16_t values per node.
 * Values 0-255 are literal byte codes, 256-510 are node indices.
 */
typedef struct
{
  uint16_t bit0,bit1;
...
/* In-memory huffman node after CAL_OptimizeNodes: values < 256 are literal
 * byte codes, values >= 256 are reinterpreted pointers to other nodes.
 * We use uintptr_t so pointers fit on 64-bit platforms.
 */
typedef struct
{
  uintptr_t bit0,bit1;
...
{
  unlink ("DEBUG.TXT");
  debughandle = open("DEBUG.TXT", O_CREAT | O_WRONLY | O_TEXT, 0644);
}
  ...
  long length,huffnode *hufftable)
{
  ...
  while (dst < dstend)
  {
    ...
    if (code < 256)
    {
      // leaf node - write decoded byte
      *dst++ = (byte)code;
      nodeon = headptr;
    }
    else
    {
      // internal node - follow pointer
      nodeon = (huffnode *)(uintptr_t)code;
    }
  }
}
...
void CAL_CarmackExpand (uint16_t far *source, uint16_t far *dest, unsigned length)
{
  uint16_t  ch,chhigh,count,offset;
  uint16_t  far *copyptr, far *inptr, far *outptr;
  unsigned char far *inptr_byte;
  ...
  while (length)
  {
    ...
    if (chhigh == NEARTAG)
    {
      ...
      {       // have to insert a word containing the tag byte
        inptr_byte = (unsigned char far *)inptr;
        ch |= *inptr_byte;
        inptr = (uint16_t far *)(inptr_byte + 1);
        *outptr++ = ch;
        length--;
      }
      else
...
long CA_RLEWCompress (uint16_t huge *source, long length, uint16_t huge *dest,
  uint16_t rlewtag)
{
  long complength;
...
void CA_RLEWexpand (uint16_t huge *source, uint16_t huge *dest,long length,
  uint16_t rlewtag)
{
  uint16_t value,count,i;
  uint16_t huge *end;
...
void CAL_SetupGrFile (void)
{
  ...
  grstarts = (int32_t _seg *)FP_SEG(&CGAhead);
#endif
...
//
  ...
  {
    huffnode_disk disk_grhuffman[255];
    int node_i;
    read(handle, &disk_grhuffman, sizeof(disk_grhuffman));
    for (node_i = 0; node_i < 255; node_i++)
    {
      grhuffman[node_i].bit0 = disk_grhuffman[node_i].bit0;
      grhuffman[node_i].bit1 = disk_grhuffman[node_i].bit1;
    }
  }
  close(handle);
  CAL_OptimizeNodes (grhuffman);
...
#ifdef PROFILE
  unlink ("PROFILE.TXT");
  profilehandle = open("PROFILE.TXT", O_CREAT | O_WRONLY | O_TEXT, 0644);
#endif
  ...
  unsigned width, unsigned height, unsigned pixshift)
{
  ...
  byte *dstptr;
  unsigned *shifttable;
  unsigned dx_count, cx_count;
  byte srcbyte;
  unsigned shifted;

  (void)segment;
  // In flat memory model, segment is a grsegs[] chunk index
  data = (byte *)grsegs[segment];
  srcptr = data + source;
  dstptr = data + dest;

  shifttable = shifttabletable[pixshift];

//
// table shift the mask
//
  for (dx_count = height; dx_count > 0; dx_count--)
  {
    *dstptr = 0xff; // first byte starts as all 1s
    for (cx_count = width; cx_count > 0; cx_count--)
    {
      srcbyte = ~(*srcptr++);
      shifted = shifttable[srcbyte];
      *dstptr &= ~(shifted & 0xff); // AND with first byte (inverted shift)
      dstptr++;
      *dstptr = ~((shifted >> 8) & 0xff); // replace next byte
    }
    dstptr++; // the last shifted byte has 1s in it
  }

//
// table shift the data (four planes)
//
  for (dx_count = height * 4; dx_count > 0; dx_count--)
  {
    *dstptr = 0; // first byte starts as 0
    for (cx_count = width; cx_count > 0; cx_count--)
    {
      srcbyte = *srcptr++;
      shifted = shifttable[srcbyte];
      *dstptr |= (shifted & 0xff); // OR with first byte
      dstptr++;
      *dstptr = (shifted >> 8) & 0xff; // replace next byte
    }
    dstptr++; // the last shifted byte has 0s in it
  }
}
...
//
  ...
  case  2:
    ...
    CAL_ShiftSprite ((unsigned)chunk,dest->sourceoffset[0],
...
{
  memset (grneeded,0,sizeof(grneeded)-1);
  ca_levelbit = 1;
  ca_levelnum = 0;
}
